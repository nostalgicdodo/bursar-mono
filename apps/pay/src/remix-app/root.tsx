...
import { RootProvider } from "@/ui/react/context-providers/root"
import type {
	LinksFunction
} from "@remix-run/node"
...
export function Layout ( { children }: { children: React.ReactNode } ) {
	...
	return <RootProvider context={{ environment: useEnv(), user: loaderData?.user }}>
...
/*
 | Consolidates the `__env` field returned from all the matching loaders
 |
 */
function useEnv () {
	// Functional version
	// return useMatches()
	// 	.filter( m => typeof m.data?.__env === "object" )
	// 	.reduce( ( acc, m ) => ({ ...acc, ...m.data.__env }), { } )

	// Imperative version
	let env = { }
	for ( let match of useMatches() ) {
		if ( ! match.data?.__env ) {
			continue
		}
		const matchEnv = match.data.__env
		for ( let k in matchEnv ) {
			env[ k ] = matchEnv[ k ]
		}
	}
	return env
