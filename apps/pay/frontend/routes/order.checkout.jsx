...
export default function () {
	...
	return <div className="container space-200-top-bottom">
		...
		{ ( ! paymentVerificationState.matches( "idle" ) ) && <>
			<section className="pay-verify space-50 columns medium-offset-2 medium-8 large-offset-3 large-6 fill-light radius-50">
				<div>
					<div className="h h4 strong text-purple-2">
						{ ( ! paymentVerificationState.matches( "verified" ) ) && <>
							Verifying payment...
						</> }
						{ paymentVerificationState.matches( "verified" ) && <>
							{ paymentVerificationState.context.mostRecentOrderStatus === "success" ? "Payment Successful" : "There was an issue" }
						</> }
					</div>
					<div className="p text-neutral-7" style={{ maxWidth: "25ch" }}>
						...
						{ ( ! paymentVerificationState.matches( "verified" ) && paymentVerificationState.context.verificationAttempts >= 2 ) && <>
		...
		<div className="user-info card columns large-6 large-offset-3 medium-8 medium-offset-2 radius-50 no-overflow">
		...
		<div className="footer" style={{ marginTop: "var(--space-200)" }}>
			<img className="institute-logo" src={`/media/logos/${order.product.brand.logoURL}`} alt="Institute logo" />
