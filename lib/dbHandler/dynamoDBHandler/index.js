...
class DynamoDBHandler {
	...
	find(params){
		...
		if(params.noPage){
			...
			return new Promise(async (resolve, reject) => {
				const {error, data} = await _this.noPageFind(params, operation);
				response(error, data, resolve, reject);
			});
		}
		if(params.withPage){
			delete params.withPage;
			// eslint-disable-next-line no-async-promise-executor
			return new Promise(async (resolve, reject) => {
				const {error, data} = await _this.withPageFind(params, operation);
				response(error, data, resolve, reject);
			});
		}
		return new Promise((resolve, reject) => {
			_this.DynamoDBClient[operation](params, (err, data) => response(err, data, resolve, reject));
		});
	}

	async withPageFind(params, operation){
		let result, accumulated = [];
		do {
			try {
				result = await this.DynamoDBClient[operation](params).promise();
			} catch(e){
				console.error(e);
				return {error: e, data: []};
			}
		...
		} while (accumulated.length < params.Limit && result.LastEvaluatedKey);
		while(accumulated.length > params.Limit){
			accumulated.pop();
		}
		return {
			error: null,
			data: {
				Items: accumulated,
				LastEvaluatedKey: this.getLastEvaluatedKey(accumulated[accumulated.length - 1], result.LastEvaluatedKey)
			}
		};
	}

	async noPageFind(params, operation){
		let result, accumulated = [];
		do {
			try {
				result = await this.DynamoDBClient[operation](params).promise();
	...
	getLastEvaluatedKey(lastItem, provisionalKey){
		if(provisionalKey){
			for(const key of Object.keys(provisionalKey)){
				provisionalKey[key] = lastItem[key];
			}
		}
